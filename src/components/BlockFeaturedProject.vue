    ...
    <div class="relative mx-auto max-w-[1580px]">
      ...
      <div class="grid gap-5 xl:grid-cols-[260px_minmax(0,1fr)] xl:items-start">
        ...
        <div class="min-w-0">
          <ProjectTicketCardClassic v-if="featuredProject" :project="featuredProject" :store-badge-src="storeBadgeSrc"
            :open-lightbox="openLightbox" :no-images-text="t('projects.noImages')" reduce-effects />
        </div>
      </div>
      ...
      <aside class="mt-6">
          ...
          <div class="grid gap-3 lg:grid-cols-[1.25fr_0.9fr_0.9fr]">
            <button
              type="button"
              class="group relative overflow-hidden rounded-[28px] border border-white/12 bg-white/6 p-2 text-left transition hover:border-white/24 focus:outline-none focus:ring-2 focus:ring-[#f4b48b]/70"
              ...
              <img :src="standGalleryUrls[0]" alt="The Tone of Victory stand 1"
                class="relative block h-[300px] w-full rounded-[22px] object-cover transition duration-300 group-hover:scale-[1.02] sm:h-[340px] lg:h-[420px]" loading="lazy" fetchpriority="low" decoding="async" />
              <div
                class="absolute left-4 top-4 rounded-full border border-white/18 bg-black/36 px-2.5 py-1 text-[10px] font-black uppercase tracking-[0.2em] text-white">
                Spring 2025
              </div>
            </button>
            ...
            <button v-for="(src, index) in standGalleryUrls.slice(1)" :key="`stand-${index + 1}`"
              ...
              <img :src="src" :alt="`The Tone of Victory stand ${index + 2}`"
                class="relative block h-[240px] w-full rounded-[22px] object-cover transition duration-300 group-hover:scale-[1.02] sm:h-[280px] lg:h-[420px]" loading="lazy" fetchpriority="low" decoding="async" />
              <div
                class="absolute left-4 top-4 rounded-full border border-white/18 bg-black/36 px-2.5 py-1 text-[10px] font-black uppercase tracking-[0.2em] text-white">
                South Korea
              </div>
